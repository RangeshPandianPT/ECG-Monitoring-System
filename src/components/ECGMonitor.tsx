...
export const ECGMonitor = () => {
  ...
  const generateECGSample = (t: number): number => {
  ...
  const startReading = async () => {
    ...
    const readableStreamClosed = portRef.current.readable.pipeTo(decoder.writable);
          ...
          if (trimmed === "!") {
            ...
            if (!isNaN(numValue)) {
              ...
              setEcgData(prev => {
                ...
                // Keep only last 500 points (about 5 seconds at 100Hz)
  ...
  useEffect(() => {
    ...
    // Use devicePixelRatio so the canvas is crisp on mobile/retina
    ...
    const draw = () => {
      ...
      // Set canvas size using DPR and set CSS size
      ...
      // Clear canvas with light medical background
      ctx.fillStyle = "hsl(var(--medical-bg))";
      ctx.fillRect(0, 0, width, height);

      // Draw grid (use device-independent px grid spacing)
